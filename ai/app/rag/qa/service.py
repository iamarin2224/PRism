...
Please provide a clear and helpful explanation based on the code above:"""
    ...
    async def stream_question(self, request: QARequest):
        import json
...
Please provide a clear and helpful explanation based on the code above:"""
        ...
        # 3. Call primary Mid-tier model with stream=True
        if settings.AICREDITS_API_KEY.strip():
            try:
                primary_client = AsyncOpenAI(
                    api_key=settings.AICREDITS_API_KEY,
                ...
                logger.warning(f"Primary MID_MODEL Q&A streaming error ({e}). Attempting OpenRouter fallback...")

        # 4. Fallback to OpenRouter with stream=True
        if not streamed_any and settings.OPENROUTER_API_KEY.strip():
            try:
                fallback_client = AsyncOpenAI(
                    api_key=settings.OPENROUTER_API_KEY,
                    base_url=settings.OPENROUTER_BASE_URL,
                ...
                logger.error(f"Fallback Q&A streaming error: {fb_err}")

        # 5. Fallback deterministic text if neither streamed
        if not streamed_any:
            if retrieval_result.chunks:
                file_list = ", ".join(list(dict.fromkeys(c.file_path for c in retrieval_result.chunks)))
                fallback_text = (
                ...
                fallback_text = f"No relevant code chunks found in repository `{request.repo_name}` for query: \"{request.query}\"."
            full_answer = fallback_text
            yield f"data: {json.dumps({'type': 'delta', 'content': fallback_text})}\n\n"
